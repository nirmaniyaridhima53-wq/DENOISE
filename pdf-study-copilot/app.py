# app.py
# PAGE 1: Research OS Landing & Upload Command Center

import base64
from pathlib import Path

import streamlit as st
import pymupdf as fitz  # modern import (no deprecation warning)

import ui_theme  # Research OS design system

# Logo lives at: pdf-study-copilot/assets/logo.png
LOGO_PATH = Path(__file__).resolve().parent / "assets" / "logo.png"
HAS_LOGO = LOGO_PATH.exists()

st.set_page_config(
    page_title="Research OS — Command Center",
    page_icon=str(LOGO_PATH) if HAS_LOGO else "🧬",
    layout="wide",
)

# ============================================================
# Design system + VISIBLE background watermark (PC-friendly)
# ============================================================
ui_theme.inject_design_system()
ui_theme.inject_logo_watermark(opacity=0.10, size="90vmin")


# ============================================================
# HEADER: Logo + Brand + Status Chip
# ============================================================
h_col1, h_col2, h_col3 = st.columns([1, 6, 2])

with h_col1:
    if HAS_LOGO:
        logo_b64 = base64.b64encode(LOGO_PATH.read_bytes()).decode("utf-8")
        st.markdown(
            f'<div class="logo-breathe">'
            f'<img src="data:image/png;base64,{logo_b64}" width="64" alt="logo"/>'
            f'</div>',
            unsafe_allow_html=True,
        )

with h_col2:
    st.markdown(
        '<h3 style="margin:0; font-family:var(--font-heading); font-weight:700; '
        'letter-spacing:-0.5px;">'
        'Research <span style="color:var(--accent-teal)">OS</span></h3>',
        unsafe_allow_html=True,
    )

with h_col3:
    ui_theme.status_chip("AI Engine Active")

st.markdown("<br>", unsafe_allow_html=True)


# ============================================================
# MAIN LAYOUT: 60% Features / 40% Upload Action
# ============================================================
col_left, col_right = st.columns([3, 2])

# ---------------------------------------------------------
# LEFT: Hero + Interactive Feature Matrix
# ---------------------------------------------------------
with col_left:
    # FIX 1: "biotech" removed
    ui_theme.hero_heading("Discover what researchers haven't tested yet")
    ui_theme.hero_sub("Upload papers. Find gaps. Run experiments.")

    st.markdown("<br>", unsafe_allow_html=True)

    row1_col1, row1_col2 = st.columns(2)
    with row1_col1:
        ui_theme.feature_card(
            "🧠",
            "Topic Map",
            "Interactive knowledge graph extraction from dense literature.",
            0,
        )
    with row1_col2:
        ui_theme.feature_card(
            "🧬",
            "Visual AI Engine",
            "Molecular diagram analysis & context-aware explanations.",
            1,
        )

    row2_col1, row2_col2 = st.columns(2)
    with row2_col1:
        ui_theme.feature_card(
            "🔍",
            "Hypothesis Gaps",
            "Identify missing links and untested variables in current research.",
            2,
        )
    with row2_col2:
        ui_theme.feature_card(
            "📝",
            "Synthesis Quiz",
            "Auto-generated comprehension testing & active recall.",
            3,
        )

# ---------------------------------------------------------
# RIGHT: Upload & Parsing OS
# ---------------------------------------------------------
with col_right:
    with st.container(border=True):
        st.markdown("#### 📂 Upload & Parsing OS")
        st.caption("Securely process PDF documents up to 50MB.")

        uploaded_file = st.file_uploader(
            "Drag & drop or click to browse",
            type=["pdf"],
            label_visibility="collapsed",
        )

        if uploaded_file is not None:
            try:
                pdf_bytes = uploaded_file.getvalue()
                doc = fitz.open(stream=pdf_bytes, filetype="pdf")
                total_pages = len(doc)

                # If a DIFFERENT pdf is uploaded, clear old study outputs
                if st.session_state.get("pdf_name") != uploaded_file.name:
                    st.session_state["selected_pages"] = []
                    st.session_state["extracted_text"] = ""
                    st.session_state["topics"] = []
                    st.session_state["youtube_links"] = []
                    st.session_state["images"] = []
                    st.session_state["image_explanations"] = {}
                    st.session_state["quiz_questions"] = []
                    st.session_state["quiz_answers"] = []
                    st.session_state["quiz_submitted"] = False

                    for key in ["page_range", "manual_start", "manual_end", "range_source"]:
                        if key in st.session_state:
                            del st.session_state[key]

                    for key in list(st.session_state.keys()):
                        if key.startswith("answer_"):
                            del st.session_state[key]

                # Store for the dashboard
                st.session_state["pdf_bytes"] = pdf_bytes
                st.session_state["pdf_name"] = uploaded_file.name
                st.session_state["total_pages"] = total_pages

                st.success(
                    f"**{uploaded_file.name}** loaded successfully "
                    f"({total_pages} pages)."
                )

                if st.button("Enter Workspace →", type="primary", width="stretch"):
                    st.switch_page("pages/1_Study_Dashboard.py")

            except Exception as e:
                st.error(f"Invalid file: {e}")
        else:
            st.info("Waiting for document input...")
            st.markdown(
                '<div style="text-align:center; color:var(--text-muted); '
                'margin-top:1rem;">'
                '<div style="font-size:3rem; opacity:0.3;">📄</div>'
                '<p>No document loaded</p>'
                '</div>',
                unsafe_allow_html=True,
            )

# ============================================================
# FOOTER
# ============================================================
st.markdown("<br><br>", unsafe_allow_html=True)
st.caption("Powered by Streamlit • Groq AI • Research OS Design System")
